...
class XBDImageDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir="/n/tambe_lab/disaster_relief/xBD_data/data",
                 transform=None,
                 mode='train'):
	if mode not in ['test', 'train', 'val']: raise ValueError('Mode must be one of \'test\', \'train\', or \'val\'. ')
	...
	self.mode = mode
        ...
        self.files = [pn for pn in os.listdir(os.path.join(self.root_dir, self.mode)) if '_post' in pn]
    ...
    def __getitem__(self, idx):
        ...
        if cur in self.labels:
            label = self.read_target_file(augmentation, os.path.join(self.root_dir, 'masks', cur))
